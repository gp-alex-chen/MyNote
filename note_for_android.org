...
1. 可以划分为模块
Fragment 要尽可能的封装起来，并且 Fragment 之间的交互
** 如何开始
...
#+END_SRC
* Loaders 
Loaders 令异步读取数据变得简单, Loaders 的一个特点就是生命周期和 Fragment 或者 Activity 同步，可以在配置改变时重连.
** Loader API Summary
...
public boolean onTouch(View v, MotionEvent event) {
    ...
    switch (action) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (upMargin >= 0){
                upMargin = 0;
            }else if (upMargin <= -buttonGo.getHeight()){
                upMargin = -buttonGo.getHeight();
            }
            layoutParams.topMargin = upMargin;
...
*** RecyclerView
**** RecyclerView 特点
1. 支持不同方向，不同排版模式，实现多种数据展现形式
2. 内部实现回收机制，简化开发
3. RecyclerView 不再包含 ListView 中的 setOnItemClickListener();
   1. 需要针对 RecyclerView 内部的元素中的控件设置 onClickListener 的形式完成点击操作，
   2. 需要给每一个点击的控件设置 onClickListener();
**** RecyclerView 点击事件的支持
因为 ViewHolder 每一次都需要重写，同时 ViewHodler 内部嫩能够通过 getAdapterPosition(), getLayoutPositon() 就可以获取当前 item 位置
*** Palette
**** API
1. Palette.from(Bitmao) 准备处理 Bitmap
2. Palette.Builder.generate() 同步方法,计算色系
3. Palette.Builder.generate(Listener) 异步方法
...
- SVN集中式版本控制
- SVN 权限管理功能目前比 Git 多。
- Git 可以通过设置公钥来省去密码输入问题
*** SVN 应用歩骤
仓库创建分为两种
1. 一个仓库就是一个项目，在仓库内部可以放很多工程，相对简单
2. 每一个仓库，在创间的时候默认创建三个文件夹 trunk(主干),branches(分支),tags(发布的版本代码不允许修改,相当于release版本)
**** 目录结构 
1. 全集中方式
2. 全集中方式2
3. 单一项目
